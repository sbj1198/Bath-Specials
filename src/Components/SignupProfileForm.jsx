...
const SignupProfileForm = () => {
  ...
  const handleAddUser = async () => {
    ...
    let result = await isUserExist(emailRef.current.value).then((result) => {
      return result;
    });
    ...
    createUser(user)
      .then((res) => {
        alert(
          "user created successfully.\npage is going to redirected to login."
        );
        navigate("/login");
      })
      .catch((err) => {
      ...
      <Grid templateColumns="repeat(3, 1fr)" gap={3} w="70%">
        ...
        <GridItem colSpan={5}>
          <FormControl>
            ...
            {showStrong && (
              <Text color={"red"}>
                password min length should be 8, password contains 1 uppercase,
                1 lowercase, 1 symbol, 1 number at least
              </Text>
            )}
